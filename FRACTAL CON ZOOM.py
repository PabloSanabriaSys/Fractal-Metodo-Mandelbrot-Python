    ...
    for y in range(alto):
        for x in range(ancho):
            ...
            if n==MAXIMO:
                ...
                #print('EL PUNTO: ', cx,', ', cy,'Pertenece :D')
            else:
...
while bandera:
  for event in pygame.event.get():
    if event.type == pygame.QUIT: # PARA SALIR DEL BUCLE Y CERRAR EL PROGRAMA
        ...
        #SELECCIONA UN PUNTO DONDE NOSOTROS DEMOS CLICK
        p = pygame.mouse.get_pos()
        px = (p[0] * (Amax - Amin) / ancho + Amin)
        py = (p[1] * (Bmin - Bmax) / alto + Bmax)
        print("({};{})".format(px,py))
        #SOBRE EL PUNTO SELECCIONADO HACEMOS UN ZOOM X10
        zoom=zoom+1
        Amin, Amax, Bmin, Bmax = px-0.**zoom, px+0.1**zoom, py-0.1**zoom, py+0.1**zoom
        iniciamos()
    elif event.type == pygame.KEYDOWN:
        if event.key == pygame.K_f:
            #APRETAMOS LA LETRA F PARA VOLVER AL ZOOM ORIGINAL
            Amin, Amax, Bmin, Bmax = -1.05, 1.05, -1.05, 1.05
            zoom=0
            iniciamos()
pygame.quit()